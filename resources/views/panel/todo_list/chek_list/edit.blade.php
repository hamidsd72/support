    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <form action="{{route('todo-list-check.update',$item->id)}}" method="POST" enctype="multipart/form-data" class="container-fluid">
                    {{method_field('PATCH')}}
                    {{csrf_field()}}
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="tab_id" required>
                                    @foreach($tabs as $tab)
                                        <option value="{{$tab->id}}" {{old('tab_id',$item->tab_id)==$tab->id?'selected':''}}>{{$tab->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
